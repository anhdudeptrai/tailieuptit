...
public class UserActionController {
    ...
    @PreAuthorize("isAuthenticated()")
    public ResponseEntity<?> downloadDocument(@PathVariable Long id, @AuthenticationPrincipal UserDetails userDetails) {
        try {
            // GỌI CHÍNH XÁC: documentService.downloadDocument()
            Resource resource = documentService.downloadDocument(id, userDetails.getUsername());

            // Lấy tên file gốc (cần logic trong service để trả về)
